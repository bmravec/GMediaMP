...
side_pane_init (SidePane *self)
{
	...
	priv->store = gtk_tree_store_new (3, G_TYPE_STRING, G_TYPE_STRING, G_TYPE_INT);
	...
	gtk_paned_add1 (GTK_PANED (self), priv->view);
		
	GtkTreeSelection *tree_selection = gtk_tree_view_get_selection (GTK_TREE_VIEW (priv->view));
	g_signal_connect (tree_selection, "changed", G_CALLBACK (selector_changed_cb), self);

	priv->notebook = gtk_notebook_new ();
	gtk_notebook_set_show_tabs (GTK_NOTEBOOK(priv->notebook), FALSE);
	gtk_paned_add2 (GTK_PANED (self), priv->notebook);
	...
	gtk_tree_view_append_column (GTK_TREE_VIEW (priv->view), column);
	gtk_tree_view_set_expander_column (GTK_TREE_VIEW (priv->view), column);
